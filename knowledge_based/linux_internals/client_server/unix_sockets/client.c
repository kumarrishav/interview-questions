...
void shutdown_handler(int signo) {
	...
	send_info(commfd, quitmsg, sizeof(quitmsg)-1);
	disconnect_from(commfd);
	exit(0);
}
...
int main(void) {
	...
	if ((commfd = connect_to("server1.example.com")) < 0) {
	...
	while (fgets(buf, sizeof(buf), stdin) != NULL) {
		...
		if (send_info(commfd, buf, line_len) < 0) {
		...
		int n;
		if ((n = recv_info(commfd, buf, sizeof(buf)-1)) < 0) {
	...
	if (send_info(commfd, buf, strlen(buf)) < 0) {
